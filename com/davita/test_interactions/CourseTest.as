...
package com.davita.test_interactions
{
	...
	import com.davita.test_interactions.TestItem;
	...
	public class CourseTest extends MovieClip {
		...
		private function answerClicked(event:MouseEvent):void
		{
			...
			if (isCorrect == "correct")
			{
				...
				dispatchEvent(new TestEvent(TestEvent.TEST_CHANGED, interaction));
			}
			else 
			{
				numOfQuestionsAnsweredIncorrectly++;
				feedbackText.text = currentTestItem.getFeedback();
				interaction = "incorrect answer";
				dispatchEvent(new TestEvent(TestEvent.TEST_CHANGED, interaction));
			}			
		...
		private function xmlLoaded(event:Event):void
		{
			...
			for (var i:int = 0; i<testXml.*.length(); i++)
			{
				...
				// TODO: Add quiz item to interactions
		...
		private function sendInteractionEvent(itemNumber:Number):void
		{
